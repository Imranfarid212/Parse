-- 40001 means a database serialization failure and infrastructure may retry it.
-- This is an application-level optimistic conflict, so return an explicit 409.
...
begin
  ...
  if p_expected_revision is not null and p_expected_revision <> v_current_revision then
    raise exception using errcode = 'PT409', message = 'receipt revision conflict';
  end if;
